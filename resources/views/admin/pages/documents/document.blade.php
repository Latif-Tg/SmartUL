...
@if(Auth::user() instanceof App\Enseignant)
    ...
    @section('title-page', 'Professeur | New')
    ...
    @section('location')
    <strong style="color:red">{{Auth::user()->name}}</strong> /  Nouveau document
    @endsection

@else
    @section('title-page', 'Faculty | New')
    ...
    @section('location')
    <strong style="color:red">{{Auth::user()->title}}</strong> / Nouveau document
    @endsection

@endif

@section('account')
<h1>Ajouter un document</h1><hr/>
@if(Auth::user() instanceof App\Faculty)
<form method="post" action="{{ route('document.admin.create') }}" enctype="multipart/form-data">
    <div class="container">
        @include('/includes.form_doc')

        <div class="text-right">
            <button type="submit" class="btn btn-dark">Ajouter<i class="material-icons" style="float:left;">file_upload</i></button>
...
@else
<form method="post" action="{{ route('document.prof.create') }}" enctype="multipart/form-data">
    <div class="container">
        @include('/includes.form_doc')
